...
### 추가
- **FCPXML 이벤트명 / 프로젝트명 직접 설정** — 결과 화면 내보내기 영역에 이벤트명·프로젝트명 입력칸 추가. 기존 하드코딩(`Auto Cut`)을 대체. 프로젝트명 기본값은 오늘 날짜(`YYYY.MM.DD`)이며 수정 가능, 이벤트명은 필수 입력(빈 칸이면 경고 후 내보내기 중단). `generate_fcpxml(event_name=, project_name=)` 키워드 인자로 전달하며, 인자 생략 시 기존 동작 유지(하위 호환).
- **인앱 사용 설명서 (❓ 도움말)** — 단계 표시줄 오른쪽 ❓ 버튼으로 빠른 시작 가이드를 띄우는 전용 다이얼로그(`ui/help_dialog.py`). 파일 추가 → 설정 → 분석 → 내보내기 4단계 흐름과 팁 안내. 세 화면 어디서나 접근 가능.
