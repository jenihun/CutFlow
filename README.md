...
작업 화면에서는 자막이 생성되는 즉시 타임라인(노랑=자막)과 로그에 실시간으로 채워집니다.

![작업 화면](docs/images/2_work.png)

| 1. 설정 | 3. 결과 |
|:---:|:---:|
| ![설정 화면](docs/images/1_settings.png) | ![결과 화면](docs/images/3_result.png) |
...
- **FCPXML 생성** — 원본을 재인코딩하지 않고 편집 정보만 저장. 비표준 해상도(2.7K/4K), 29.97fps, DJI 내장 타임코드까지 정확히 처리해 FCP가 오류 없이 가져옵니다. **이벤트명·프로젝트명을 직접 지정**할 수 있습니다(프로젝트명 기본값은 오늘 날짜).
...
- **인앱 도움말 (❓)** — 화면 어디서나 단계 표시줄 오른쪽 ❓ 버튼으로 빠른 시작 가이드를 볼 수 있습니다.
...
컷 기준 선택   자막 로그 스트리밍         이벤트·프로젝트명 입력
Whisper 옵션  (클릭 → 위치 강조)        FCPXML / SRT 내보내기
                                    AI 제목·썸네일 추천
```

> 단계 표시줄 오른쪽의 **❓ 버튼**으로 어느 화면에서든 사용법을 볼 수 있습니다.
...
3. **결과** 화면에서 이벤트명·프로젝트명을 입력하고 `FCPXML 내보내기` → Final Cut Pro로 바로 열어 편집을 이어갑니다.
...
│   └── help_dialog.py        # 인앱 사용 설명서(❓) 다이얼로그
└── docs/
    ├── FEATURES.md           # 기능 명세
    └── CHANGELOG.md          # 버전 이력
```
